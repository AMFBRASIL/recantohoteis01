    ...
    @if($rows->total() > 0)
        @foreach($rows as $row)
            <tr class="{{$row->status}}">
                ...
                <td class="title"> <a href="{{route($route_list['edit'],['parent'=>$row->cost_center_id, 'id'=>$row->id])}}">{{$row->name}}</a></td>
                <td>{{ display_date($row->updated_at)}}</td>
                <td><span class="badge badge-{{ $row->status }}">{{ $row->status }}</span></td>
                <td>
                    <a href="{{route($route_list['edit'],['parent'=>$row->cost_center_id, 'id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Editar')}}
                    </a>
                </td>
            </tr>
        @endforeach
