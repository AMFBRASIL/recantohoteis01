        ...
        <div class="row">
            ...
            <div class="col-md-8">
                <div class="filter-div d-flex justify-content-between ">
                    <div class="col-left">
                        @if(!empty($rows))
                            <form method="post" action="{{route('product_unity.admin.bulkEdit')}}" class="filter-form filter-form-left d-flex justify-content-start">
                                {{csrf_field()}}
                    ...
                    <div class="col-left">
                        <form method="get" action="{{route('product_unity.admin.index')}} " class="filter-form filter-form-right d-flex justify-content-end" role="search">
                ...
                <div class="panel">
                    <div class="panel-title">{{__("Todas Unidades")}}</div>
                    <div class="panel-body">
                                ...
                                @if(count($rows) > 0)
                                    @foreach($rows as $row)
                                        <tr>
                                            ...
                                            <td class="title"><a href="{{route('product_unity.admin.edit',['id'=>$row->id])}}">{{$row->acronym}}</a></td>
                                            <td class="title">{{$row->description}}</td>
                                            <td>
                                                <a href="{{route('product_unity.admin.edit',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Edit')}}
                                                </a>
