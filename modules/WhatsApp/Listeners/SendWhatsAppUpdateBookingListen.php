...
class SendWhatsAppUpdateBookingListen
{
    /**
    ...
    public function handle(BookingCreatedEvent $event)
    {
        ...
        if (!empty($booking->phone) and !empty($booking->country) and !empty(setting_item('enable_whatsApp_user_has_booking'))) {
            $message = $this->replaceMessage($booking, setting_item_with_lang('whatsApp_message_user_when_booking', app()->getLocale()));
            try {
        ...
        if (!empty($vendor->phone) and !empty($vendor->country) and !empty(setting_item('enable_whatsApp_vendor_has_booking'))) {
            $message = $this->replaceMessage($booking, setting_item_with_lang('whatsApp_message_vendor_when_booking', app()->getLocale()));
            try {
        ...
        if (!empty($adminPhone) and !empty($adminCountry) and !empty(setting_item('enable_whatsApp_admin_has_booking')) and !empty(setting_item('admin_phone_has_booking'))) {
            $message = $this->replaceMessage($booking, setting_item_with_lang('whatsApp_message_admin_when_booking', app()->getLocale()));
            try {
